...
class MLPPolicy(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
    ...
    def get_action(self, obs: np.ndarray) -> np.ndarray:
        ...
        inp = torch.from_numpy(observation).float().to(ptu.device)
        if self.discrete:
            # Return action
            ac = torch.argmax(self.logits_na(inp)).item()
        else:
            mean_val = self.mean_net(inp)
            print(self.logstd)
            samples = torch.normal(mean_val, torch.exp(self.logstd))
    ...
    def forward(self, observation: torch.FloatTensor):
        ...
        ##################################
        if self.discrete:
            # Return action logits as a categorical distribution
            logits = self.logits_na(observation)
            probs = logits / torch.sum(logits)
            action_distribution = distributions.Categorical(probs=probs)
        else:
            mean_val = self.mean_net(torch.from_numpy(observation))
            action_distribution = distributions.MultivariateNormal(mean_val, torch.exp(self.logstd))
        ##################################
...
class MLPPolicyPG(MLPPolicy):
    ...
    def update(self, observations, actions, advantages, q_values=None):
        ...
        #loss = TODO
        #####################################################
        # J_ML(theta) =~ 1/N sum_{i=1}^N ( sum_{t=1}^T ( log pi_theta( a_{i,t} | s_{i,t} ) ) )
        # N = observations.shape[0]
        # Vectorized operations
        print('self.forward(observations).log_prob(actions): ', self.forward(observations).log_prob(actions))
        # loss = - 1 / N * ...
        ...
        # TODO
        #####################################################
        # This is always the same for every pytorch/tensorflow optimization:
        print('>> loss:', loss)
